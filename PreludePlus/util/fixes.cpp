...
MemoryUnprotector::MemoryUnprotector(uint32_t dllAddress, size_t size) noexcept
	: mAddress(ptd::Dll::GetInstance().GetAddress(dllAddress)), mSize(size), mOldProtection(0) {
...
void TempleFix::write(uint32_t offset, const void* buffer, size_t size) {
	...
	memcpy(ptd::Dll::GetInstance().GetAddress(offset), buffer, size);
}

void TempleFix::read(uint32_t offset, void* buffer, size_t size) {
	memcpy(buffer, ptd::Dll::GetInstance().GetAddress(offset), size);
}
...
void TempleFix::writeHex(uint32_t offset, const string &hexPattern) {
	...
	memcpy(ptd::Dll::GetInstance().GetAddress(offset), buffer, totalSize);
}

void *TempleFix::replaceFunctionInternal(uint32_t offset, void* replaceWith) {
	void* original = nullptr;
	auto target = ptd::Dll::GetInstance().GetAddress(offset);
...
void TempleFix::redirectCall(uint32_t offset, void* redirectTo) {
	...
	// Read what's there...
	auto realAddress = ptd::Dll::GetInstance().GetAddress(offset);
...
void TempleFix::breakRegion(uint32_t from, uint32_t to)
{
	...
	auto realFrom = reinterpret_cast<char*>(ptd::Dll::GetInstance().GetAddress(from));
...
void TempleFix::writeCall(uint32_t offset, void* redirectTo) {
	...
	auto startOfNextInstruction = reinterpret_cast<int32_t>(ptd::Dll::GetInstance().GetAddress(offset + 5));
